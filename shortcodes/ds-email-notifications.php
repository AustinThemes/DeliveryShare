...
function WDstatusAlert($ds_order_id, $ds_email_address_to_alert, $ds_new_status){
		...
		if($ds_notification_message && $ds_notification_subject_line && $ds_notification_multiple_to_recipients){ 
			wp_mail( $ds_notification_multiple_to_recipients, $ds_notification_subject_line, $ds_notification_message );
			return '<h2>Customer notified of status change.</h2>';
		} else {return '<h2>Customer not notified.</h2>';}
